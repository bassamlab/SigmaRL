...
from sigmarl.helper_scenario import get_distances_between_agents, get_perpendicular_distances
...
class WorldStateRTReal(WorldStateRT):
    ...
    def reset(self, agent_states: List[AgentState]):
        ...
        for i_agent in range(self.n_agents):
            ...
            self._reset_agent_related_ref_path(slice(None),
                                               i_agent,
                                               ref_path,
                                               path_id,
                                               extended_points)

            self.reset_init_distances_and_short_term_ref_path(agent_states,
                                                              slice(None),
                                                              i_agent)
    ...
    def _reset_init_state(self,
        ...
        distances = [get_perpendicular_distances(agent_pos, ref_path["center_line"]) for ref_path in ref_paths_scenario]
        ...
        rel_yaws = [get_relative_yaw(agent_rot,
    ...
    def update_mutual_distances(self, agent_states, env_index=slice(None)):
        mutual_distances = get_distances_between_agents(
            data=torch.stack(
                [agent_states[i].pos for i in range(self.n_agents)]
            )
