...
from sigmarl.helper_scenario import Distances, get_perpendicular_distances, get_rectangle_vertices
...
class WorldStateGR(WorldState):
    ...
    def update_distances(self, agent_states: List[AgentState], agent_index: int, env_index=slice(None)):
        ...
        (self.distances.ref_paths[env_index, 0], _,) = get_perpendicular_distances(
            ...
            polyline=torch.stack([self.params.original_pos[env_index], self.params.goal[env_index]], dim=1),
        )
