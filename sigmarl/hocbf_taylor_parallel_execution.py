...
class HOCBF:
    def __init__(
        ...
        self.a_x_min, self.a_x_max = -np.inf, np.inf
        self.a_y_min, self.a_y_max = -np.inf, 10
...
def conservative_aggressive_class_k():
    ...
    for idx, lambda_1 in enumerate(lambda_1s):
        ...
        t_idx_where = np.where(
            (hocbf.v_y_vals != v_y_target) | (hocbf.v_x_vals != v_x_target)
        )[0]
...
def run_single_experiment(para_tuple):
    ...
    print(
        f"Running experiment {idx} with parameters: DT={DT}, relative_degree={relative_degree}, approach={approach}, lambda_1={lambda_1}, lambda_2={lambda_2}, lambda_3={lambda_3}"
    )
    ...
    #         lambda_1, lambda_2, lambda_3 = 3.0, None, None
    #     elif relative_degree == 2:
    #         lambda_1, lambda_2, lambda_3 = 5.0, 10.0, None
    #     elif relative_degree == 3:
    #         lambda_1, lambda_2, lambda_3 = 5.0, 6.0, 8.0
    # else:
    #     lambda_1, lambda_2, lambda_3 = 0.1, None, None
...
def run_experiment_1_parallel(
    ...
    param_list = [p + (SIM_DURATION,) for p in param_list]
...
def plot_heatmap(data_name, fig_name, lambda_1_list, lambda_2_list):
    """
    Plots a heatmap (2D) or bar plot (1D) depending on the availability of lambda_2_list.

    Parameters:
        data_name (str): Path to the pickle file containing the data.
        fig_name (str): Name for saving the figure.
        lambda_1_list (list): List of lambda_1 values.
        lambda_2_list (list or None): List of lambda_2 values. If None or empty, a 1D plot is generated.
    """
    ...
    if lambda_2_list[0] is None:
        ...
        ax2.set_xticklabels(
            [f"{data_array[i]:.2f}" for i in selected_indices]
        )  # Match lambda ticks
        ...
        # Set log-scaled ticks for scientific readability
        # ax.set_xscale("log")
        # ax.set_yscale("log")
        ax.set_xticks(
            np.linspace(min(lambda_1_list), max(lambda_1_list), num=num_ticks)
...
def plot_footprint(
    ...
    for idx in selected_indices:
        ...
        if approach.lower() == "hocbf":
            ...
            label_fp = rf"Our $(\lambda_1 = {{{lambda_1:.3f}}})$"
...
def plot_h(
    ...
    colormap = plt.colormaps.get_cmap(
        "viridis"
    )  # Use same colormap as heatmap (change if needed)
    norm = colors.Normalize(
        vmin=np.nanmin(avg_x_speed_list), vmax=np.nanmax(avg_x_speed_list)
    )  # Normalize speed values
    ...
    for idx in selected_indices:
        ...
        if approach.lower() == "hocbf":
            if relative_degree == 1:
                ...
                label_h = rf"$h (\lambda_1 = {{{lambda_1:.2f}}}, \lambda_2={{{lambda_2:.2f}}})$"
                label_h_lb = rf"$h_{{\mathrm{{lb}}}} (\lambda_1 = {{{lambda_1:.2f}}}, \lambda_2={{{lambda_2:.2f}}})$"
            elif relative_degree == 3:
                label_h = rf"$h (\lambda_1 = {{{lambda_1:.2f}}}, \lambda_2={{{lambda_2:.2f}}}, \lambda_3={{{lambda_3:.2f}}})$"
                label_h_lb = rf"$h_{{\mathrm{{lb}}}} (\lambda_1 = {{{lambda_1:.2f}}}, \lambda_2={{{lambda_2:.2f}}}, \lambda_3={{{lambda_3:.2f}}})$"
        else:
            label_h = rf"$h (\lambda_1 = {{{lambda_1:.3f}}})$"
            label_h_lb = rf"$h_{{\mathrm{{lb}}}} (\lambda_1 = {{{lambda_1:.3f}}})$"
        ...
        #     elif relative_degree == 2:
        #         label_fp = (
        #             rf"HOCBF $(\lambda_1 = {{{lambda_1:.2f}}}, \lambda_2={{{lambda_2:.2f}}})$"
        #         )
        #     elif relative_degree == 3:
        #         label_fp = rf"HOCBF $(\lambda_1 = {{{lambda_1:.2f}}}, \lambda_2={{{lambda_2:.2f}}}, \lambda_3={{{lambda_3:.2f}}})$"
        # else:
...
if __name__ == "__main__":
    ...
    if is_example:
        ...
        data_name = "motivation_example.pkl"
        SIM_DURATION = 2.0
    else:
        approach_list = ["hocbf"]
        # approach_list=["taylor"]
        data_name = f"experiment_data_{approach_list[0].lower()}.pkl"
        SIM_DURATION = 3.0

    if approach_list[0].lower() == "hocbf":
        lambda_1_list = np.logspace(0.322, 1.25, 10)
        lambda_2_list = np.logspace(-1, 1.2, 10)
    else:
        if is_example:
            lambda_1_list = [0.01, 0.1, 0.5]
            # lambda_1_list=[0.005, 0.04, 0.5]
        else:
            lambda_1_list = np.logspace(-3, -0.3, 20)
        lambda_2_list = [None]

    lambda_3_list = [None]

    run_experiment_1_parallel(
        DT_list=DT_list,
